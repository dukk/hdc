...
export async function listClumpCatalog(installRoot, allowedVerbs) {
  ...
  for (const m of manifests) {
    ...
    const servicesIdx = tierParts.lastIndexOf("services");
    const infraIdx = tierParts.lastIndexOf("infrastructure");
    const clientsIdx = tierParts.lastIndexOf("clients");
    let tier = null;
    if (servicesIdx >= 0 && servicesIdx === tierParts.length - 2) tier = "service";
    else if (infraIdx >= 0 && infraIdx === tierParts.length - 2) tier = "infrastructure";
    else if (clientsIdx >= 0 && clientsIdx === tierParts.length - 2) tier = "client";
    if (!tier) continue;
    packages.push({
      tier,
      id: manifestId(m),
