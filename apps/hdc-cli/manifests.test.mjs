...
describe("manifests", () => {
  ...
  it("parseRunTier and manifestRunTier map CLI tier to directory", () => {
    root = mkdtempSync(join(tmpdir(), "hdc-manifests-"));
    writeTree(root, {
      "clumps/services/svc/manifest.json": JSON.stringify({ id: "svc", verbs: {} }),
      "clumps/clients/win/manifest.json": JSON.stringify({ id: "win", verbs: {} }),
      "clumps/infrastructure/pve/manifest.json": JSON.stringify({ id: "pve", verbs: {} }),
    });
    const m = discoverManifests(join(root, "clumps"));
  ...
  it("resolveRunInvocation routes legacy platform manifest", () => {
    root = mkdtempSync(join(tmpdir(), "hdc-manifests-"));
    writeTree(root, {
      "clumps/clients/legacy/manifest.json": JSON.stringify({
        id: "legacy",
        platforms: ["windows"],
        verbs: { maintain: { script: "run.mjs" } },
      }),
    });
    const m = manifestById(discoverManifests(join(root, "clumps")), "legacy");
  ...
  it("verbSpec rejects invalid specs", () => {
    root = mkdtempSync(join(tmpdir(), "hdc-manifests-"));
    writeTree(root, {
      "clumps/services/q/manifest.json": JSON.stringify({
        id: "q",
        verbs: {
          deploy: "not-object",
          maintain: { script: "" },
    ...
    const m = discoverManifests(join(root, "clumps"))[0];
