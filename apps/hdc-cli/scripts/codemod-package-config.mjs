...
function collectRunMjs(dir) {
  ...
  for (const rel of ["clumps/services", "clumps/infrastructure"]) {
    walk(join(dir, rel));
  }
  return out;
}
...
function importPathFor(filePath) {
  const libPath = join(root, "clumps", "lib", "clump-run-config.mjs");
  return relative(dirname(filePath), libPath).replace(/\\/g, "/");
}

function patchFile(content, filePath) {
  if (content.includes("clump-run-config.mjs")) return content;
  ...
  const importLine = `import { loadClumpConfigFromClumpRoot, tryLoadClumpConfigFromClumpRoot } from "${importPath}";\n`;
  ...
  if (!/\bconst clumpRoot\b/.test(next) && /\bconst here\b/.test(next)) {
    next = next.replace(
      /(const here = dirname\(fileURLToPath\(import\.meta\.url\)\);[^\n]*\n)/,
      `$1const clumpRoot = join(here, "..");\n`,
    );
  }
...
`;
  ...
  return loaded;
      ...
      if (!next.includes("ensurePackageConfig") && !next.includes("CLUMP_CONFIG_EXAMPLE")) {
