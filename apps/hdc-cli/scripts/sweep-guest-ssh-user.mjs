...
function relImport(file) {
  ...
  let rel = path.relative(dir, path.join(clumpsRoot, "lib/guest-ssh-resolve.mjs")).replace(/\\/g, "/");
...
const skip = new Set([
  path.join(clumpsRoot, "lib/guest-ssh-resolve.mjs"),
  path.join(clumpsRoot, "lib/guest-ssh-exec.mjs"),
]);
