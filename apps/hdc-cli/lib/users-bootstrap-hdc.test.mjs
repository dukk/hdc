...
describe("bootstrapHostDocsFromInfrastructureConfigs", () => {
  it("loads ubuntu/proxmox package config bootstrap_hosts with proxmox/ubuntu tags", () => {
    ...
    try {
      mkdirSync(join(root, "clumps/infrastructure/ubuntu"), { recursive: true });
      mkdirSync(join(root, "clumps/infrastructure/proxmox"), { recursive: true });
      writeFileSync(
        join(root, "clumps/infrastructure/ubuntu/config.json"),
        JSON.stringify({
          schema_version: 1,
          bootstrap_hosts: [
      ...
      writeFileSync(
        join(root, "clumps/infrastructure/proxmox/config.json"),
        JSON.stringify({
          schema_version: 1,
