...
function ensurePackageDir(publicRoot) {
  mkdirSync(join(publicRoot, "clumps/services/uptime-kuma"), { recursive: true });
}
...
describe("uptime-kuma config write", () => {
  ...
  it("migrateUptimeKumaConfigToSplitLayout writes sidecars and include index", () => {
    ...
    expect(
      JSON.parse(
        readFileSync(join(publicRoot, "clumps/services/uptime-kuma/monitors/pi-hole-a.json"), "utf8"),
      ),
  ...
  it("writeUptimeKumaConfig preserves split layout and removes orphan monitor files", () => {
    ...
    writeFileSync(
      join(publicRoot, "clumps/services/uptime-kuma/monitors/orphan.json"),
      '{"id":"orphan"}',
      "utf8",
    );
    ...
    expect(
      existsSync(join(publicRoot, "clumps/services/uptime-kuma/monitors/orphan.json")),
    ).toBe(false);
    expect(
      existsSync(join(publicRoot, "clumps/services/uptime-kuma/monitors/bind-a.json")),
    ).toBe(false);
    expect(
      JSON.parse(
        readFileSync(join(publicRoot, "clumps/services/uptime-kuma/monitors/pi-hole-a.json"), "utf8"),
  ...
  it("writeUptimeKumaConfig falls back to flat layout when not split", () => {
    ...
    expect(
      existsSync(join(publicRoot, "clumps/services/uptime-kuma/monitors/pi-hole-a.json")),
    ).toBe(false);
  });
});
