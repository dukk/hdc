import { rebootQemuGuest } from "hdc/clump/infrastructure/proxmox/lib/proxmox-guest-resources.mjs";
import { qemuFirstBootWaitOptsFromConfig } from "hdc/clump/infrastructure/proxmox/lib/proxmox-provision-config.mjs";
...
function resolveProxmoxCfg(opts) {
  ...
  if (typeof opts.proxmoxPackageRoot === "string" && opts.proxmoxPackageRoot.trim()) {
    try {
      return loadClumpConfigFromClumpRoot(opts.proxmoxPackageRoot.trim()).data;
    } catch {
      return undefined;
    }
  }
