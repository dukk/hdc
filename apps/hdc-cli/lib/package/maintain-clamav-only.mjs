...
/**
 * Collect SSH targets from clump config (deployments[] or top-level configure).
 ...
 * @param {string} opts.clumpRoot clumps/services/<id>
...
export async function runClamavOnlyMaintain(opts) {
  const { target, verb, clumpRoot, root } = opts;
  ...
  if (!loaded.ok || !loaded.data) {
    const payload = {
      ...
      target,
      verb,
      message: "clump config missing — guest baseline not applied",
      results: [],
    };
