...
import { maybeNotifyOpsDiscordFromOperationReport } from "../ops-discord-notify.mjs";
import { preferredClumpReportPath } from "../private-repo.mjs";
import {
 ...
 * @returns {string}
 */
export function defaultOperationReportPath(clumpRoot, verb, reportPathArg, publicRoot) {
...
export function defaultNextSteps(ctx) {
  ...
  if (verb === "deploy") {
    ...
    for (const r of results) {
      ...
      if (deployIp) {
        lines.push(
          `Update \`operations/inventory/systems/${sid}.json\` → \`access.nodes[0].ip\` to \`${deployIp}\` if not already set.`,
        );
      } else {
        lines.push(
          `Set \`access.nodes[0].ip\` in \`operations/inventory/systems/${sid}.json\` after the guest has an address.`,
        );
      }
    }
    ...
    lines.push("Update or remove related `operations/inventory/systems/*.json` sidecars if the system is retired.");
...
function renderAccessMarkdown(ctx) {
  ...
  for (const inv of ctx.inventory) {
    ...
    for (const n of nodes) {
      ...
      else if (ip && ctx.clumpId === "pi-hole") {
...
export function renderOperationReportMarkdown(ctx, reportPathForHeader, extraSections) {
  ...
  const title = `${ctx.clumpTitle} ${ctx.verb} report`;
...
export function writeOperationReportFile(opts) {
  const { clumpRoot, ctx, reportPathArg, extraSections, repoRoot } = opts;
 ...
 */
export function loadPackageManifestForReport(clumpRoot) {
  const path = join(clumpRoot, "manifest.json");
  if (!existsSync(path)) {
...
export function runOperationReportTail(opts) {
  const manifest = loadPackageManifestForReport(opts.clumpRoot);
  const ctx =
    opts.reportCtx ??
    createOperationReportContext({
      clumpId: manifest.id,
      clumpTitle: manifest.title,
      verb: opts.verb,
      argv: opts.argv,
      manifestNextSteps: manifest.nextSteps,
    });
  return finalizeOperationReport({
    clumpRoot: opts.clumpRoot,
    ctx,
...
export function finalizeOperationReport(opts) {
  const { clumpRoot, ctx, repoRoot, payload, ok, log, extraSections, reportPathArg } = opts;
