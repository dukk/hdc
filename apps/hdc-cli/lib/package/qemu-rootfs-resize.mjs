...
export function growRootFilesystemScript() {
  return [
    ...
    "  partprobe \"$ROOT_PART\" 2>/dev/null || blockdev --rereadpt \"$ROOT_PART\" 2>/dev/null || true",
    "  export TMPDIR=/var/tmp",
    '  if ! growpart "$ROOT_PART" "$ROOT_NUM" > /tmp/hdc-growpart.out 2>&1; then',
    "    grep -q 'NOCHANGE:' /tmp/hdc-growpart.out || { cat /tmp/hdc-growpart.out >&2; exit 1; }",
    "  fi",
