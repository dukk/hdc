/**
 * Parse root `.env.example` into global vs per-clump sections (by `# clumps/...` headers).
 */
...
export const GLOBAL_ENV_KEYS = new Set([
  ...
  "HDC_SSH_COMMAND_TIMEOUT_MS",
  "HDC_PVE_HTTP_TIMEOUT_MS",
]);

/**
 * @typedef {{ packageRel: string | null; clumpId: string | null; lines: string[] }} EnvExampleSection
 */
 ...
 * @returns {string | null}
 */
export function clumpIdFromRel(packageRel) {
...
export function parseEnvExampleSections(text) {
  ...
  const globalLines = [
    ...
    "# Package-specific variables live under clumps/<tier>/<id>/.env (see each package .env.example).",
    "",
  ];
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    if (keyMatch) {
      ...
      if (GLOBAL_ENV_KEYS.has(key)) {
        ...
        const id = clumpIdFromRel(currentPackageRel);
