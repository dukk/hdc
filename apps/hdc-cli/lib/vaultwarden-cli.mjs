...
/**
 * Bitwarden session for the current hdc process (after unlock). Used to pass BW_SESSION to child clump scripts.
 * @returns {string | null}
 */
 ...
 * @param {VaultwardenCliDeps} deps
 * @param {string[]} args
 * @param {{ capture?: boolean; session?: string; password?: string; apiKey?: { clientId: string; clientSecret: string }; allowMissing?: boolean; stdin?: string }} [opts]
 */
function spawnBw(deps, bwArgs, opts = {}) {
  ...
  if (opts.apiKey) {
    env.BW_CLIENTID = opts.apiKey.clientId;
    env.BW_CLIENTSECRET = opts.apiKey.clientSecret;
  }
...
/**
 * @param {string[]} urls
 * @returns {Array<{ uri: string; match: null }>}
 */
export function normalizeLoginUris(urls) {
  /** @type {string[]} */
  const seen = [];
  for (const raw of urls) {
    const uri = String(raw ?? "").trim();
    if (!uri || seen.includes(uri)) continue;
    seen.push(uri);
  }
  return seen.map((uri) => ({ uri, match: null }));
}
 ...
 * @returns {string[]}
 */
export function loginUrisFromBwItem(item) {
 ...
 * @param {string} session
 * @param {string} itemId
 * @param {{ password?: string; uris?: string[] }} patch
 */
function bwUpdateLoginItem(deps, session, itemId, patch) {
  const item = bwGetItem(deps, session, itemId);
  const login =
    item.login && typeof item.login === "object"
      ? /** @type {Record<string, unknown>} */ ({ ...item.login })
      : {};
  if (typeof patch.password === "string") login.password = patch.password;
  if (Array.isArray(patch.uris)) login.uris = normalizeLoginUris(patch.uris);
  item.login = login;
...
function bwUpdateLoginPassword(deps, session, itemId, password) {
  bwUpdateLoginItem(deps, session, itemId, { password });
}

/**
 * @param {VaultwardenCliDeps} deps
 * @param {string} session
 * @param {{ name: string; password: string; organizationId: string; collectionId: string; uris?: string[] }} opts
...
function bwCreateOrgLoginItem(deps, session, opts) {
  const item = {
    ...
    name: opts.name,
    login: {
      uris: normalizeLoginUris(opts.uris ?? []),
      username: opts.name,
 ...
 */
function bwLoginApiKey(deps, clientId, clientSecret) {
  deps.log("[hdc] vaultwarden: logging in with API key");
  const r = spawnBw(deps, ["login", "--apikey"], {
    capture: true,
    apiKey: { clientId, clientSecret },
  });
  if (!r.ok) {
    throw new Error(`bw login --apikey failed: ${r.stderr || r.stdout || "invalid API key"}`);
  }
}

/**
 * @param {VaultwardenCliDeps} deps
 * @param {(key: string) => Promise<string | null>} readLocalSecret
 * @returns {Promise<{ clientId: string; clientSecret: string } | null>}
 */
async function resolveVaultwardenApiKeyCredentials(deps, readLocalSecret) {
  let clientId = String(deps.env.HDC_VAULTWARDEN_KEY_CLIENT_ID ?? "").trim();
  let clientSecret = String(deps.env.HDC_VAULTWARDEN_KEY_CLIENT_SECRET ?? "").trim();
  const email = String(deps.env.HDC_VAULTWARDEN_EMAIL ?? "").trim();
  if (!clientId && !clientSecret && email) {
    return null;
  }
  if (!clientId) {
    const stored = await readLocalSecret("HDC_VAULTWARDEN_KEY_CLIENT_ID");
    if (typeof stored === "string" && stored.length > 0) clientId = stored;
  }
  if (!clientSecret) {
    const stored = await readLocalSecret("HDC_VAULTWARDEN_KEY_CLIENT_SECRET");
    if (typeof stored === "string" && stored.length > 0) clientSecret = stored;
  }
...
async function ensureBwUnlockedInner(deps, readLocalSecret, writeLocalSecret) {
  ...
  if (!url) {
    throw new Error("HDC_VAULTWARDEN_URL must be set for the vaultwarden secret backend");
  }
  ...
  const apiKeyCreds = await resolveVaultwardenApiKeyCredentials(deps, readLocalSecret);
  const email = String(deps.env.HDC_VAULTWARDEN_EMAIL ?? "").trim();
  const useApiKey = apiKeyCreds !== null;

  if (!useApiKey && !email) {
    throw new Error(
      "Vaultwarden auth required: set HDC_VAULTWARDEN_KEY_CLIENT_ID and HDC_VAULTWARDEN_KEY_CLIENT_SECRET, or HDC_VAULTWARDEN_EMAIL",
    );
  }
   ...
   * @param {string} pass
   */
  function ensureLoggedIn(pass) {
    if (bwLoginCheck(deps).ok) return;
    if (useApiKey && apiKeyCreds) {
      bwLoginApiKey(deps, apiKeyCreds.clientId, apiKeyCreds.clientSecret);
      return;
    }
    bwLogin(deps, email, pass);
  }

  ensureLoggedIn(masterPassword);
 ...
 * @param {string} itemName
 * @param {string} value
 * @param {{ uris?: string[] }} [opts]
 */
export function bwSetPassword(deps, session, itemName, value, opts = {}) {
  const { organizationId, collectionId } = resolveBwOrgContext(deps, session);
  const uris = Array.isArray(opts.uris) ? opts.uris : undefined;
  ...
  if (orgItemId) {
    try {
      /** @type {{ password: string; uris?: string[] }} */
      const patch = { password: value };
      if (uris) patch.uris = uris;
      bwUpdateLoginItem(deps, session, orgItemId, patch);
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
  ...
  if (personalItemId) {
    ...
    try {
      ...
      const patch = { password: value };
      if (uris) patch.uris = uris;
      bwUpdateLoginItem(deps, session, personalItemId, patch);
    } catch (e) {
      const msg = e instanceof Error ? e.message : String(e);
      throw new Error(`bw edit item failed for ${itemName}: ${msg}`);
    }
 ...
 * @returns {string[]}
 */
export function bwGetLoginUris(deps, session, itemName) {
  const { organizationId } = resolveBwOrgContext(deps, session);
  const cachedItem = bwFindOrgFullItem(deps, session, itemName, organizationId);
  if (cachedItem) return loginUrisFromBwItem(cachedItem);
  const itemId = cachedItem && typeof cachedItem.id === "string" ? cachedItem.id : bwFindItemId(deps, session, itemName, organizationId);
  if (!itemId) return [];
  return loginUrisFromBwItem(bwGetItem(deps, session, itemId));
}
 ...
 * @param {string[]} uris
 */
export function bwUpdateLoginUris(deps, session, itemName, uris) {
  const { organizationId } = resolveBwOrgContext(deps, session);
  const cachedItem = bwFindOrgFullItem(deps, session, itemName, organizationId);
  let itemId =
  ...
  if (!itemId) {
    throw new Error(`bw update uris failed for ${itemName}: item not found in organization`);
  }
  bwUpdateLoginItem(deps, session, itemId, { uris });
  processBwOrgFullItems = null;
  processBwOrgItems = null;
}
