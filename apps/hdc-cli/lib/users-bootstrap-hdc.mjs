...
/**
 * Vault entry for the local `hdc` user password, keyed by host `id` (from sidecar or clump config).
...
/**
 * Host entries from `bootstrap_hosts` in infrastructure clump config.json files
...
export function bootstrapHostDocsFromInfrastructureConfigs(root, deps) {
  ...
  for (const pkg of ["windows", "ubuntu", "raspberrypi"]) {
    const rel = `clumps/clients/${pkg}/config.json`;
  ...
  for (const { tier, pkg } of sources) {
    const rel = `clumps/${tier}/${pkg}/config.json`;
    ...
    for (const h of hosts) {
      ...
      const src =
        resolved.source === "private"
          ? `clumps/${tier}/${pkg}/config.json (hdc-private)`
          : `clumps/${tier}/${pkg}/config.json`;
      out.push({
        label: `${src}#${id || `bootstrap_hosts[${idx}]`}`,
        data: rec,
      });
...
export async function runUsersBootstrapHdc(argv, deps, options = {}) {
  ...
  if (work.length === 0) {
    deps.warn(
      "users bootstrap-hdc: no bootstrap_hosts in clumps/infrastructure/{ubuntu,proxmox} or clumps/clients/<id>/config.json (or pass --sidecar).",
    );
    return;
  }
