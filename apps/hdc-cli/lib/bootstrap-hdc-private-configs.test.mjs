...
describe("bootstrap-hdc-private-configs", () => {
  ...
  beforeEach(() => {
    ...
    const pkgDir = join(publicRoot, "clumps", "services", "foo");
  ...
  it("creates config.json from example on first run", () => {
    const dest = join(privateRoot, "clumps", "services", "foo", "config.json");
  ...
  it("skips existing config.json without --force", () => {
    const dest = join(privateRoot, "clumps", "services", "foo", "config.json");
    mkdirSync(join(privateRoot, "clumps", "services", "foo"), { recursive: true });
