...
describe("json-config-preprocess", () => {
  ...
  describe("resolveIncludeRelPath", () => {
    it("resolves relative to base file directory", () => {
      expect(resolveIncludeRelPath("clumps/services/bind/config.json", "zones/a.json")).toBe(
        "clumps/services/bind/zones/a.json",
      );
    });

    it("rejects path escape", () => {
      expect(() =>
        resolveIncludeRelPath("clumps/services/bind/config.json", "../../../../etc/passwd"),
  ...
  describe("expandHdcIncludes", () => {
    ...
    it("splices array includes", () => {
      const rel = "clumps/infrastructure/cloudflare/config.json";
      mkdirSync(join(publicRoot, "clumps/infrastructure/cloudflare/zones"), { recursive: true });
      writeFileSync(
        join(publicRoot, "clumps/infrastructure/cloudflare/zones/a.json"),
        '{"name":"a"}',
        "utf8",
      );
      writeFileSync(
        join(publicRoot, "clumps/infrastructure/cloudflare/zones/b.json"),
        '[{"name":"b1"},{"name":"b2"}]',
        "utf8",
      );
    ...
    it("supports nested includes in fragment files", () => {
      ...
      mkdirSync(join(publicRoot, "clumps/services/bind/fragments"), { recursive: true });
      writeFileSync(
        join(publicRoot, "clumps/services/bind/fragments/records.json"),
        '[{"type":"A","name":"x","data":"1.2.3.4"}]',
        "utf8",
      );
      writeFileSync(
        join(publicRoot, "clumps/services/bind/fragments/zone.json"),
        `{
  "id": "example.test",
  "zone_type": "forward",
  "records": [{ "$hdc.include": "records.json" }]
    ...
    it("prefers public over private for included files", () => {
      const rel = "clumps/services/pi-hole/config.json";
      mkdirSync(join(publicRoot, "clumps/services/pi-hole"), { recursive: true });
      mkdirSync(join(privateRoot, "clumps/services/pi-hole"), { recursive: true });
      writeFileSync(join(publicRoot, rel), '{"items":[{"$hdc.include":"part.json"}]}', "utf8");
      writeFileSync(
        join(publicRoot, "clumps/services/pi-hole/part.json"),
        '{"from":"public"}',
        "utf8",
      );
      writeFileSync(
        join(privateRoot, "clumps/services/pi-hole/part.json"),
        '{"from":"private"}',
        "utf8",
      );
    ...
    it("errors on circular includes", () => {
      const rel = "clumps/services/n8n/config.json";
      mkdirSync(join(publicRoot, "clumps/services/n8n"), { recursive: true });
      writeFileSync(
        join(publicRoot, rel),
      ...
      writeFileSync(
        join(publicRoot, "clumps/services/n8n/loop.json"),
    ...
    it("accepts object form with file property", () => {
      const rel = "clumps/services/gatus/config.json";
      mkdirSync(join(publicRoot, "clumps/services/gatus"), { recursive: true });
      writeFileSync(join(publicRoot, "clumps/services/gatus/extra.json"), '{"ok":true}', "utf8");
      writeFileSync(
        join(publicRoot, rel),
    ...
    it("loads BOM-prefixed include sidecars", () => {
      const rel = "clumps/services/uptime-kuma/config.json";
      mkdirSync(join(publicRoot, "clumps/services/uptime-kuma/monitors-public"), {
        recursive: true,
      });
      writeFileSync(
        join(publicRoot, "clumps/services/uptime-kuma/monitors-public/immich.json"),
  ...
  describe("readResolvedPackageConfigJson integration", () => {
    ...
    beforeEach(() => {
      publicRoot = mkdtempSync(join(tmpdir(), "hdc-public-"));
      packageRoot = join(publicRoot, "clumps/infrastructure/cloudflare");
    ...
    it("loads via loadClumpConfigFromClumpRoot with comments and includes", () => {
      writeFileSync(
  ...
  "zones": [
    { "$hdc.include": "zones/example.com.json" },
      ...
      const { data } = loadClumpConfigFromClumpRoot(packageRoot, { publicRoot });
    ...
    it("readResolvedPackageConfigJson honors preprocess:false", () => {
      ...
      const resolved = resolveRepoFile(publicRoot, "clumps/infrastructure/cloudflare/config.json");
