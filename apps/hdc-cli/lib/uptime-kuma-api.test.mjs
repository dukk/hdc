...
import {
  ...
  notificationListRowsFromPayload,
...
describe("uptime-kuma-api notificationList", () => {
  it("notificationListRowsFromPayload converts object map to array", () => {
    expect(notificationListRowsFromPayload({ 1: { id: 1, name: "Discord" } })).toEqual([
      { id: 1, name: "Discord" },
    ]);
  });

  it("waitForNotificationListEvent resolves on notificationList socket event", async () => {
    /** @type {import("socket.io-client").Socket} */
    const socket = {
      once(event, handler) {
        expect(event).toBe("notificationList");
        setTimeout(() => handler({ 2: { id: 2, name: "HDC Ops Discord", type: "discord" } }), 10);
      },
    };
    const list = await waitForNotificationListEvent(socket, 1000);
    expect(list).toEqual({ 2: { id: 2, name: "HDC Ops Discord", type: "discord" } });
  });
});
