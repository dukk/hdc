...
const validCfg = {
  schema_version: 2,
  bind_config_path: "clumps/services/bind/config.json",
  defaults: {
    mode: "proxmox-lxc",
    proxmox: {
      lxc: {
...
describe("nagios deployments", () => {
  ...
  it("requires bind_config_path and deployments", () => {
    ...
    expect(() =>
      normalizeNagiosConfig({ schema_version: 2, bind_config_path: "clumps/services/bind/config.json" }),
    ).toThrow(/deployments/);
  });
  ...
  it("normalizes valid v2 config", () => {
    ...
    expect(norm.bindConfigPath).toBe("clumps/services/bind/config.json");
  });
