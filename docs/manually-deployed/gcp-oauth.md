...
Google Auth Platform OAuth 2.0 web clients (`*.apps.googleusercontent.com`) for Sign in with Google on self-hosted apps are declared and validated with the **gcp-oauth** infrastructure package (`packages/infrastructure/gcp-oauth/`).
...
Copy `packages/infrastructure/gcp-oauth/config.example.json` to **hdc-private** as `config.json` (same path).
...
```bash
node tools/hdc/cli.mjs secrets set HDC_GCP_OAUTH_VAULTWARDEN_CLIENT_ID
node tools/hdc/cli.mjs secrets set HDC_GCP_OAUTH_VAULTWARDEN_CLIENT_SECRET
```

Or import from a Console download in one step:

```bash
node tools/hdc/cli.mjs run infrastructure gcp-oauth maintain -- --import ./client_secret_123.json
```
...
1. **Validate / checklist:** `node tools/hdc/cli.mjs run infrastructure gcp-oauth maintain -- --dry-run`
...
5. **Import to vault:** `node tools/hdc/cli.mjs run infrastructure gcp-oauth maintain -- --import ./downloaded.json`
6. **Verify:** `node tools/hdc/cli.mjs run infrastructure gcp-oauth query -- --import ./downloaded.json --require-vault`
...
```bash
node tools/hdc/cli.mjs run infrastructure gcp-oauth query --
node tools/hdc/cli.mjs run infrastructure gcp-oauth query -- --import ./client.json --require-vault
node tools/hdc/cli.mjs run infrastructure gcp-oauth maintain -- --dry-run
node tools/hdc/cli.mjs run infrastructure gcp-oauth maintain -- --import ./client.json
```
...
- Reverse proxy hostnames: `packages/services/nginx-waf/`
- [AGENTS.md](../../AGENTS.md)
