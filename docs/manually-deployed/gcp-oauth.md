...
```bash
node apps/hdc-cli/cli.mjs secrets set HDC_GCP_OAUTH_VAULTWARDEN_CLIENT_ID
node apps/hdc-cli/cli.mjs secrets set HDC_GCP_OAUTH_VAULTWARDEN_CLIENT_SECRET
```

Or import from a Console download in one step:

```bash
node apps/hdc-cli/cli.mjs run infrastructure gcp-oauth maintain -- --import ./client_secret_123.json
```
...
1. **Validate / checklist:** `node apps/hdc-cli/cli.mjs run infrastructure gcp-oauth maintain -- --dry-run`
...
5. **Import to vault:** `node apps/hdc-cli/cli.mjs run infrastructure gcp-oauth maintain -- --import ./downloaded.json`
6. **Verify:** `node apps/hdc-cli/cli.mjs run infrastructure gcp-oauth query -- --import ./downloaded.json --require-vault`
...
node apps/hdc-cli/cli.mjs run infrastructure gcp-oauth query -- --import ./client.json --require-vault
node apps/hdc-cli/cli.mjs run infrastructure gcp-oauth maintain -- --dry-run
node apps/hdc-cli/cli.mjs run infrastructure gcp-oauth maintain -- --import ./client.json
```
