...
Live or plugin-generated facts accumulate in **`inventory/automated/systems.json`** (updated on successful `hdc run <tier> <package> query|deploy`). Resolve a system id with automated overlay first, then manual: use `resolveSystemById` from `tools/hdc/inventory.mjs` or merge behavior documented in `hdc help run`.
