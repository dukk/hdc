...
export function resolveBwExecutable(deps) {
  ...
  if (process.platform === "win32") {
    ...
    candidates.push(join(npmDir, "bw.cmd"), join(npmDir, "bw.exe"));
  }
...
function spawnBw(deps, args, opts = {}) {
  ...
  const ok = r.status === 0;
  if (!ok && !opts.allowMissing && stderr && !opts.capture) {
    deps.warn(`[hdc] bw ${args.slice(1).join(" ")} failed: ${stderr.split("\n")[0]}`);
  }
  return { ok, status: r.status ?? 1, stdout, stderr };
}
...
export function ensureBwConfigured(deps, url) {
  const bw = resolveBwExecutable(deps);
  deps.log(`[hdc] vaultwarden: configuring bw server ${url}`);
  const r = spawnBw(deps, [bw, "config", "server", url], { capture: true });
  if (!r.ok) {
...
function bwLoginCheck(deps) {
  const bw = resolveBwExecutable(deps);
  return spawnBw(deps, [bw, "login", "--check"], { capture: true, allowMissing: true });
}
...
function bwLogin(deps, email, masterPassword) {
  ...
  const r = spawnBw(deps, [bw, "login", email, masterPassword, "--raw"], { capture: true, password: masterPassword });
  if (!r.ok) {
...
function bwUnlockRaw(deps, masterPassword) {
  const bw = resolveBwExecutable(deps);
  const r = spawnBw(deps, [bw, "unlock", "--passwordenv", "BW_PASSWORD", "--raw"], {
    capture: true,
...
export function bwGetPassword(deps, session, itemName) {
  const bw = resolveBwExecutable(deps);
  const r = spawnBw(deps, [bw, "get", "password", itemName], { capture: true, session });
  if (!r.ok) {
...
function bwFindItemId(deps, session, itemName) {
  const bw = resolveBwExecutable(deps);
  const r = spawnBw(deps, [bw, "list", "items", "--search", itemName], { capture: true, session });
...
export function bwSetPassword(deps, session, itemName, value) {
  ...
  if (existingId) {
    const r = spawnBw(
      deps,
      [bw, "edit", "item", existingId, "--password", value],
  ...
  const r = spawnBw(
    deps,
    [bw, "create", "item", "login", "--name", itemName, "--username", itemName, "--password", value],
    { capture: true, session },
  );
  if (!r.ok) {
...
export function bwListItemNames(deps, session) {
  const bw = resolveBwExecutable(deps);
  const r = spawnBw(deps, [bw, "list", "items"], { capture: true, session });
...
export function bwDeleteItem(deps, session, itemName) {
  ...
  if (!id) return false;
  const bw = resolveBwExecutable(deps);
  const r = spawnBw(deps, [bw, "delete", "item", id], { capture: true, session });
  if (!r.ok) {
