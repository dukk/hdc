...
describe("runDailyMaintain", () => {
  ...
  it("dry-run plans invocations without spawning", async () => {
    ...
    writeTree(root, {
      "packages/services/kafka/manifest.json": miniManifest("kafka", "services"),
      "packages/services/kafka/config.json": JSON.stringify({ schema_version: 1 }),
      "packages/services/kafka/query/run.mjs": "process.exit(0)\n",
    });
  ...
  it("continues after a failure and exits non-zero", async () => {
    ...
    writeTree(root, {
      "packages/services/kafka/manifest.json": miniManifest("kafka", "services"),
      "packages/services/kafka/config.json": JSON.stringify({ schema_version: 1 }),
      "packages/services/kafka/query/run.mjs": "process.stdout.write(JSON.stringify({ok:false})+'\\n'); process.exit(1)\n",
      "packages/services/homeassistant/manifest.json": miniManifest("homeassistant", "services"),
      "packages/services/homeassistant/config.json": JSON.stringify({ schema_version: 1 }),
      "packages/services/homeassistant/query/run.mjs":
