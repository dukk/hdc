...
import { resolvePackageConfig } from "./package-config.mjs";
...
function stepHasConfig(deps, root, step) {
  ...
  const packageRoot = configPackageRoot(deps, root, step.tier, step.id);
  const resolved = resolvePackageConfig(root, packageRoot, "config.json", deps.env);
...
function defaultReportPath(root, basename, env) {
  return preferredNewFilePath(root, join("tools", "hdc", "reports", basename).replace(/\\/g, "/"), env);
}
