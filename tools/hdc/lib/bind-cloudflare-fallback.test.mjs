...
} from "../../../packages/services/bind/lib/bind-cloudflare-fallback.mjs";
...
describe("bind-cloudflare-fallback", () => {
  ...
  it("local A record overrides Cloudflare A at same name", () => {
    const local = [{ type: "A", name: "vault", data: "10.0.0.99", ttl: 3600 }];
    const cf = [{ type: "A", name: "vault", data: "99.129.209.234", ttl: 3600 }];
    const merged = mergeCloudflareFallbackRecords(local, cf, "dukk.org");
    expect(merged.filter((r) => r.name === "vault")).toEqual([
      { type: "A", name: "vault", data: "10.0.0.99", ttl: 3600 },
    ]);
  });

  it("merges Cloudflare vault when not defined locally", () => {
    const local = [{ type: "A", name: "bind-a.hdc", data: "10.0.0.2", ttl: 3600 }];
    const cf = [
  ...
  it("local CNAME owner blocks conflicting Cloudflare A", () => {
    const local = [{ type: "CNAME", name: "ha", data: "ha.hdc.dukk.org", ttl: 3600 }];
    const cf = [{ type: "A", name: "ha", data: "99.129.209.234", ttl: 3600 }];
    const merged = mergeCloudflareFallbackRecords(local, cf, "dukk.org");
    expect(merged.filter((r) => r.name === "ha")).toEqual([
      { type: "CNAME", name: "ha", data: "ha.hdc.dukk.org", ttl: 3600 },
    ]);
  });
  ...
  it("exclude_types skips Cloudflare NS records when loading", () => {
        ...
        zones: [
          {
            name: "dukk.org",
            records: [
              { type: "NS", name: "hdc", data: "ns-a.dukk.org", ttl: 1 },
    ...
    const records = loadCloudflareFallbackRecords(
      root,
      { zone: "dukk.org", config_path: "packages/infrastructure/cloudflare/config.json" },
      "dukk.org",
    );
