...
describe("cli run client platform packages", () => {
  ...
  it("runs packages/clients/windows query script", async () => {
    root = mkdtempSync(join(tmpdir(), "hdc-cli-windows-"));
    const pkgDir = join(root, "packages", "clients", "windows");
    ...
    const code = await runCli(
      ...
      createMemoryCliDeps({ root, packagesDir: (r) => join(r, "packages"), capture }),
    );
    expect(code).toBe(0);
