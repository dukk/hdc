 ...
 * Usage: node tools/hdc/scripts/rollout-admin-password.mjs [--dry-run]
 */
import { existsSync, readdirSync } from "node:fs";
import { join } from "node:path";

import { ensureAdminUser } from "../../../packages/lib/admin-user-ensure.mjs";
import { createConfigureExec } from "../../../packages/services/postfix-relay/lib/postfix-relay-configure.mjs";
import { resolveGuestSshUser } from "../../../packages/lib/guest-ssh-resolve.mjs";
import { createPackageVaultAccess } from "../../../packages/lib/package-vault-access.mjs";
import { provisionLogFromConsole } from "../../../packages/lib/host-provisioner.mjs";
import { resolvePveSshForHost } from "../../../packages/services/ollama/lib/ollama-install.mjs";
...
const servicesRoot = join(privateRoot, "packages", "services");
 ...
 * @returns {import("../../../packages/lib/clamav-ensure.mjs").ConfigureExec | null}
 */
function resolveExecForDeployment(deployment) {
  ...
  if (hostId && Number.isFinite(lxcVmid) && lxcVmid > 0) {
    const pveSsh = resolvePveSshForHost(
      join(root, "packages", "infrastructure", "proxmox"),
      proxmoxHostId,
    );
...
/** @type {string[]} */
const packageIds = readdirSync(servicesRoot, { withFileTypes: true })
...
async function main() {
  ...
  for (const pkg of packageIds) {
    ...
    let cfg = null;
    try {
      const packageRoot = join(root, "packages", "services", pkg);
      const loaded = loadPackageConfigFromPackageRoot(packageRoot, { publicRoot: root });
      cfg = loaded.data;
    } catch (e) {
