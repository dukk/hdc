#!/usr/bin/env node
/**
 * Split root `.env.example` into global + per-package `.env.example` files.
 * Regenerates tools/hdc/lib/env-key-packages.mjs.
 *
 * Usage: node tools/hdc/scripts/split-env-example.mjs [--write]
 */
...
function inferPackageRel(pkgId) {
  for (const tier of ["infrastructure", "services", "clients"]) {
    if (existsSync(join(publicRoot, "packages", tier, pkgId, "manifest.json"))) {
      return `${tier}/${pkgId}`;
    }
  }
...
function splitLegacyEnvExample(text) {
  ...
  const globalOut = [
    ...
    "# Package-specific variables: packages/<tier>/<id>/.env (see each .env.example).",
    "",
  ];
  ...
  for (const line of text.split(/\r?\n/)) {
    ...
    if (currentRel) {
      ensureSection(currentRel).push(line);
      if (key) {
        const id = packageIdFromRel(currentRel);
...
function writeEnvKeyPackages(keyToPackageId) {
  ...
  writeFileSync(join(publicRoot, "tools/hdc/lib/env-key-packages.mjs"), lines.join("\n"), "utf8");
}
...
function main() {
  ...
  for (const [rel, lines] of packageSections) {
    const dest = join(publicRoot, "packages", rel, ".env.example");
    mkdirSync(dirname(dest), { recursive: true });
    writeFileSync(dest, `${lines.join("\n").trimEnd()}\n`, "utf8");
    console.error(`wrote packages/${rel}/.env.example`);
  }
  writeEnvKeyPackages(keyToPackageId);
  console.error("wrote tools/hdc/lib/env-key-packages.mjs");
