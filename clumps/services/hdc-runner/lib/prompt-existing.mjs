...
export async function promptExistingGuestAction(systemId, vmid, node, name) {
  ...
  const q = `${systemId}: ${label}vmid ${vmid} on ${node} already exists. [s]kip / [r]edeploy? `;
  if (!stdin.isTTY) {
    stderr.write(
      `[hdc] hdc-runner: not a TTY — skipping ${systemId} (use --redeploy-existing or --skip-existing).\n`,
    );
    return "skip";
  }
