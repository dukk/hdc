...
async function sendDiscordNotification(opts) {
  ...
  const opsDiscordUrl = pathToFileURL(
    join(INSTALL_ROOT, "apps/hdc-cli/lib/ops-discord-notify.mjs"),
  ).href;
  const vaultAccessUrl = pathToFileURL(
    join(INSTALL_ROOT, "apps/hdc-cli/lib/vault-access.mjs"),
  ).href;
  const nodeCliDepsUrl = pathToFileURL(
    join(INSTALL_ROOT, "apps/hdc-cli/lib/node-cli-deps.mjs"),
  ).href;
...
async function runAgentSchedule(opts) {
  ...
  const libRoot = join(INSTALL_ROOT, "clumps/services/hdc-runner/lib");
  let exitCode = 1;
  let stderr = "";
  let stdout = "";
...
async function main() {
  ...
  const reportEmailUrl = pathToFileURL(
    join(INSTALL_ROOT, "clumps/lib/report-email.mjs"),
  ).href;
