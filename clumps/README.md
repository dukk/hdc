...
Plugins under `clumps/{clients,infrastructure,services}/` automate home data center operations via the hdc CLI. Each package has a `manifest.json`, optional `config.json` (copy from `config.example.json` in hdc-private), and `deploy/`, `maintain/`, `query/`, and sometimes `teardown/` scripts.
...
|----------|-----------|---------|
| `client` | `clumps/clients/` | `run client windows query --` |
| `infrastructure` | `clumps/infrastructure/` | `run infrastructure proxmox query --` |
| `service` | `clumps/services/` | `run service pi-hole maintain --` |

- **Config:** `clumps/<tier-dir>/<dir>/config.json` in hdc-private (public repo has `config.example.json` only).
- **Inventory:** `inventory/manual/{systems,services,targets}/` sidecars in hdc-private.
- **Details:** per-clump README below; full operator reference in [AGENTS.md](../AGENTS.md).
- **Schemas:** [`apps/hdc-cli/schema/`](../apps/hdc-cli/schema/).
