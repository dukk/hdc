 ...
 * @param {string} opts.proxmoxPackageRoot packages/infrastructure/proxmox
 * @param {string} [opts.packageId] service package manifest id
...
export async function syncProxmoxGuestTagsOnMaintain(opts) {
  const { deployment, proxmoxPackageRoot, packageId, flags = {} } = opts;
  const log = opts.log ?? (() => {});
  ...
  if (!packageId || !String(packageId).trim()) {
    return { ok: true, skipped: true, message: "no package id" };
  }
  ...
  if (dryRun) {
    log(`[dry-run] would ensure ${guestType} package tag ${JSON.stringify(packageId)} on ${hostId}`);
    return { ok: true, dry_run: true, guest_type: guestType, host_id: hostId, package_id: packageId };
  }
