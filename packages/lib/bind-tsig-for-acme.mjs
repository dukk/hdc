...
/**
 * Authoritative BIND TSIG from bind package config (hdc-private when present).
 * @param {string} repoRoot
 */
export function loadBindTsigSecretFromConfig(repoRoot) {
  const bindRoot = join(repoRoot, "packages", "services", "bind");
  const { data } = loadPackageConfigFromPackageRoot(bindRoot, {
    exampleRel: "packages/services/bind/config.example.json",
  });
