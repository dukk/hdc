...
/**
 * Load client_defaults from packages/services/postfix-relay/config.json (hdc-private first).
 * @param {object} [opts]
...
export function loadMailRelayClientDefaults(opts = {}) {
  ...
  const root = repoRoot();
  const packageRoot = join(root, "packages", "services", "postfix-relay");
  ...
  try {
    const loaded = loadPackageConfigFromPackageRoot(packageRoot, {
      exampleRel: "packages/services/postfix-relay/config.example.json",
    });
