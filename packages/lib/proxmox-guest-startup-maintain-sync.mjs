...
/**
 * Sync onboot + startup order from package config onto a live Proxmox guest.
 *
 * @param {object} opts
 ...
 * @param {string} [opts.packageId] service package manifest id for priority fallback
...
export async function syncProxmoxGuestStartupOnMaintain(opts) {
  ...
  let proxmoxCfg = null;
  try {
    proxmoxCfg = loadPackageConfigFromPackageRoot(proxmoxPackageRoot);
  } catch {
    proxmoxCfg = null;
  }
