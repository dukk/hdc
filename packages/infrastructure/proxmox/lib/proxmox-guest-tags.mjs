 ...
 * @returns {string | null}
 */
export function normalizePackageTag(packageId) {
  if (typeof packageId !== "string") return null;
  const tag = packageId.trim().toLowerCase();
 ...
 */
export function mergePackageTag(existing, packageId) {
  const tag = normalizePackageTag(packageId);
  if (!tag) {
...
export async function ensureGuestPackageTag(opts) {
  const { guestType, apiBase, authorization, rejectUnauthorized, node, vmid, packageId } = opts;
  const log = opts.log ?? (() => {});
  const tag = normalizePackageTag(packageId);
  if (!tag) {
    return { ok: false, changed: false, message: `invalid package tag ${JSON.stringify(packageId)}` };
  }
