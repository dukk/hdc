 ...
 */
export function parseGuestBootOptions(block, proxmoxCfg, packageId) {
  ...
  if (!startup && packageId && proxmoxCfg) {
    const priorities = startupPrioritiesFromConfig(proxmoxCfg);
    const order = priorities[packageId];
    if (order !== undefined) {
 ...
 * @param {string} [packageId]
 */
export function guestBootOptsFromBlock(block, proxmoxCfg, packageId) {
  return parseGuestBootOptions(block, proxmoxCfg, packageId);
}
...
export async function applyGuestBootOptionsFromBlock(opts) {
  const boot = parseGuestBootOptions(opts.block, opts.proxmoxCfg, opts.packageId);
