...
export async function runProxmoxHostLoadReport(opts) {
  const { packageRoot, log, warn = log, vault } = opts;
  const configRel = "packages/infrastructure/proxmox/config.json";

  if (!existsSync(join(packageRoot, "config.json"))) {
  ...
  const data = await collectProxmoxCapacityReport({ packageRoot, warn, vault });
