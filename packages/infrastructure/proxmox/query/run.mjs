/**
 * Query Proxmox VE nodes from `packages/infrastructure/proxmox/config.json` (clusters + hosts),
...
async function main() {
  ...
  if (!existsSync(configPath)) {
    errout.write(
      `[proxmox] Missing ${configRel} — copy packages/infrastructure/proxmox/config.example.json to config.json.\n`,
    );
    process.exitCode = 1;
    return;
  }
  ...
  if (!clusterKeys.length) {
    errout.write(
      `[proxmox] No Proxmox hypervisors in packages/infrastructure/proxmox/config.json (clusters[].hosts[] with id, pve_node, web_ui, ip, ssh).\n`,
    );
    process.exitCode = 1;
    return;
  }
  ...
  logUser(
    `Found ${clusterKeys.length} cluster/standalone group(s) from package config: ${clusterKeys.map((k) => JSON.stringify(k)).join(", ")}`,
  );
  ...
  const payload = {
    ...
    message:
      "Proxmox snapshot from packages/infrastructure/proxmox/config.json — systems[] on stdout only (inventory paths not used).",
  };
