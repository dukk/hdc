...
/**
 * Resolve project_dir relative to the cloudflare-workers package root (public then hdc-private).
 * @param {string} packageRoot
 * @param {string} projectDirRel
 */
export function resolveWorkerProjectDir(packageRoot, projectDirRel) {
  const rel = projectDirRel.replace(/\\/g, "/").replace(/^\.\//, "");
  const packageRel = `packages/infrastructure/cloudflare-workers/${rel}`;
  const resolved = resolveRepoFile(repoRoot(), packageRel);
  if (resolved.found) return resolved.path;

  const fallback = join(packageRoot, rel);
...
export const PACKAGE_CONFIG_EXAMPLE = "packages/infrastructure/cloudflare-workers/config.example.json";
