...
export async function deployWorker(opts) {
  const log = opts.log ?? (() => {});
  const projectPath = resolveWorkerProjectDir(opts.packageRoot, opts.worker.project_dir);
  const wranglerEnv = {
...
export async function deployPages(opts) {
  const log = opts.log ?? (() => {});
  const projectPath = resolveWorkerProjectDir(opts.packageRoot, opts.page.project_dir);
  const wranglerEnv = {
    CLOUDFLARE_API_TOKEN: opts.token,
    CLOUDFLARE_ACCOUNT_ID: opts.config.accountId,
  };
