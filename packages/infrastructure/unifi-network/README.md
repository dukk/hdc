...
node tools/hdc/cli.mjs run infrastructure unifi-network maintain --
...
```bash
node tools/hdc/cli.mjs run infrastructure unifi-network query -- --import-port-forwards --yes
```
...
- Schema: [`tools/hdc/schema/unifi-network.config.schema.json`](../../../tools/hdc/schema/unifi-network.config.schema.json)
