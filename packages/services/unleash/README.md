...
```bash
node tools/hdc/cli.mjs run service unleash deploy -- --instance a
node tools/hdc/cli.mjs run service unleash query -- --live
node tools/hdc/cli.mjs run service unleash maintain --
```
...
1. **CT IP:** from deploy/query `upstream_url` (e.g. `http://10.0.0.123:4242`).
...
- Schema: [`tools/hdc/schema/unleash.config.schema.json`](../../../tools/hdc/schema/unleash.config.schema.json)
