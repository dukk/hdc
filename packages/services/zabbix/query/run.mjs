...
import { listZabbixDeploymentSummaries, normalizeZabbixConfig, resolveZabbixDeployments } from "../lib/deployments.mjs";
import { resolvePveSshForHost } from "../lib/zabbix-install.mjs";
import { queryZabbixInCt, queryZabbixOnHost } from "../lib/query-status.mjs";
...
function loadCfg() {
  ...
  if (loaded.missing) {
    const example = tryLoadPackageConfigFromPackageRoot(packageRoot, { filename: "config.example.json" });
...
async function main() {
  ...
  const rel = relative(
    root,
    loaded.ok ? loaded.path : join(packageRoot, "config.example.json"),
  ).replace(/\\/g, "/");
