...
```bash
node tools/hdc/cli.mjs secrets set HDC_GREENBONE_ADMIN_PASSWORD
node tools/hdc/cli.mjs run service greenbone deploy --
node tools/hdc/cli.mjs run service greenbone query -- --live
```
...
3. Run `node tools/hdc/cli.mjs run service greenbone maintain --`.
