...
Copy [`config.example.json`](config.example.json) to `packages/services/lms/config.json` in **hdc-private** (or public hdc for local dev).
...
```bash
node tools/hdc/cli.mjs run service lms deploy -- --instance a --destroy-existing
node tools/hdc/cli.mjs run service lms maintain --
node tools/hdc/cli.mjs run service lms query -- --live
node tools/hdc/cli.mjs run service lms teardown -- --instance a --yes
```
