...
```bash
node tools/hdc/cli.mjs secrets set HDC_GLOBALPING_ADOPTION_TOKEN
node tools/hdc/cli.mjs run service globalping deploy -- --instance a
node tools/hdc/cli.mjs run service globalping query -- --live
node tools/hdc/cli.mjs run service globalping maintain --
```
...
2. **BIND:** add `globalping-a` A record in hdc-private `packages/services/bind/config.json` (IP from deploy output / `ip-allocations.md`; apply via `bind maintain`).
