...
- **Config:** copy [`config.example.json`](config.example.json) to `config.json` in hdc-private (`packages/services/rustdesk/config.json`)
...
```bash
node tools/hdc/cli.mjs run service rustdesk deploy -- --instance a
node tools/hdc/cli.mjs run service rustdesk query -- --live
node tools/hdc/cli.mjs run service rustdesk maintain --
```
...
- Schema: [`tools/hdc/schema/rustdesk.config.schema.json`](../../../tools/hdc/schema/rustdesk.config.schema.json)
