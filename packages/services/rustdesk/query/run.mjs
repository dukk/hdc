...
import { resolvePveSshForHost } from "../lib/rustdesk-install.mjs";
import { queryRustdeskInCt } from "../lib/query-status.mjs";
import { loadPackageConfigFromPackageRoot, tryLoadPackageConfigFromPackageRoot } from "../../../lib/package-run-config.mjs";

const here = dirname(fileURLToPath(import.meta.url));
const packageRoot = join(here, "..");
const PACKAGE_CONFIG_EXAMPLE = "packages/services/rustdesk/config.example.json";
...
async function main() {
  ...
  if (!cfg && loaded.missing) {
    const examplePath = join(root, PACKAGE_CONFIG_EXAMPLE);
    try {
      const raw = JSON.parse(readFileSync(examplePath, "utf8"));
      if (isObject(raw)) {
        cfg = raw;
        rel = PACKAGE_CONFIG_EXAMPLE;
        loaded = { ...loaded, ok: true, missing: false };
        errout.write(`[hdc] ${target} ${verb}: using ${PACKAGE_CONFIG_EXAMPLE} (no config.json yet).\n`);
      }
    } catch {
      /* fall through */
    }
