...
1. **Proxmox** — `packages/infrastructure/proxmox/config.json` with target host (e.g. `pve-a`) and API token in vault.
...
Copy [`config.example.json`](config.example.json) to **hdc-private**: `packages/services/windows-desktop/config.json`.
...
```bash
node tools/hdc/cli.mjs run service windows-desktop deploy -- \
