...
```bash
node tools/hdc/cli.mjs secrets set HDC_VAULTWARDEN_ADMIN_TOKEN
node tools/hdc/cli.mjs run vaultwarden deploy -- --instance a
node tools/hdc/cli.mjs run vaultwarden query -- --live
node tools/hdc/cli.mjs run vaultwarden maintain --
```
...
4. **nginx-waf:** add a site with `proxy_pass` to the CT upstream; enable WebSockets if needed (`websocket_enabled: true` in config).
