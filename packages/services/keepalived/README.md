...
Copy [`config.example.json`](config.example.json) to hdc-private `packages/services/keepalived/config.json`.
...
1. `node tools/hdc/cli.mjs run service keepalived query -- --live` — confirm MASTER holds VIP.
...
```bash
node tools/hdc/cli.mjs run service keepalived deploy --
node tools/hdc/cli.mjs run service keepalived maintain --
node tools/hdc/cli.mjs run service keepalived query -- --live
```
