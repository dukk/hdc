...
async function loadManifestTools(installRoot) {
  const manifestsPath = join(installRoot, "tools/hdc/manifests.mjs");
  const pathsPath = join(installRoot, "tools/hdc/paths.mjs");
  const m = await import(pathToFileURL(manifestsPath).href);
  const p = await import(pathToFileURL(pathsPath).href);
  return { ...m, packagesDir: p.packagesDir };
}
 ...
 */
export async function listPackageCatalog(installRoot, allowedVerbs) {
  const { discoverManifests, manifestId, manifestTitle, VERBS, packagesDir } =
  ...
  const manifests = discoverManifests(packagesDir(installRoot));
 ...
 * @param {string[]} allowedVerbs
 */
export async function validatePackageRun(installRoot, tier, packageId, verb, allowedVerbs) {
  const { manifestByTierAndId, discoverManifests, VERBS, packagesDir } =
    await loadManifestTools(installRoot);
  ...
  const manifests = discoverManifests(packagesDir(installRoot));
  const found = manifestByTierAndId(manifests, tier, packageId);
  if (!found) {
    return { ok: false, error: `package not found: ${tier}/${packageId}` };
  }
  return { ok: true, tier, package: packageId, verb: verbNorm };
}
