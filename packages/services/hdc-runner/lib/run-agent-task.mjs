...
async function main() {
  ...
  const tasksUrl = pathToFileURL(
    join(INSTALL_ROOT, "packages/services/hdc-runner/lib/hdc-runner-tasks.mjs"),
  ).href;
  const agentRunUrl = pathToFileURL(
    join(INSTALL_ROOT, "packages/services/hdc-runner/lib/hdc-runner-agent-run.mjs"),
  ).href;
  const jobsUrl = pathToFileURL(
    join(INSTALL_ROOT, "packages/services/hdc-runner/lib/hdc-runner-ui-jobs.mjs"),
  ).href;
