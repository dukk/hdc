...
```bash
node tools/hdc/cli.mjs secrets set HDC_HDC_RUNNER_UI_PASSWORD
```
...
```bash
node tools/hdc/cli.mjs run service hdc-runner maintain -- --test-discord
node tools/hdc/cli.mjs run service hdc-runner maintain -- --test-schedule monitor-uptime-kuma
```
...
3. If logs show `bw login failed`, re-run `maintain` to refresh `HDC_VAULTWARDEN_MASTER_PASSWORD` on the guest.
