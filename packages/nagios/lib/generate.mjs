...
import { manualSystemInventoryFileName } from "../../../tools/hdc/lib/inventory-naming.mjs";
import { NAGIOS_CENTRAL_SYSTEM_ID } from "../../lib/deploy-inventory.mjs";
...
export function resolveCentral(cluster) {
  ...
  if (!address) {
    throw new Error(
      `inventory/manual/systems/${manualSystemInventoryFileName(NAGIOS_CENTRAL_SYSTEM_ID)}: set nagios.central.address to your Nagios host IP (NRPE allowed_hosts + check target).`,
    );
  }
