...
import { NAGIOS_CLUSTER_NODE_IDS } from "../../lib/deploy-inventory.mjs";
import {
  nagiosRepoRoot,
  NAGIOS_CENTRAL_SYSTEM_ID,
  PRIMARY_PROXMOX_CLUSTER_INVENTORY_ID,
  primaryClusterInventoryPath,
} from "../lib/repo.mjs";
...
function main() {
  ...
  console.error(
    `[hdc] Nagios deploy: ok — ${bundle.stats.hostCount} hosts, ${bundle.stats.serviceCount} services, ${hypervisors.length} NRPE nodes (cluster ${NAGIOS_CLUSTER_NODE_IDS.join(", ")}, central ${NAGIOS_CENTRAL_SYSTEM_ID}).`,
  );
}
